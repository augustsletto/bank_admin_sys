...
# from models import db, seedData, Customer, Account, Transaction, TransactionOperation, TransactionType, AccountType, Role, User, roles_users
# import numpy as np
...
# from app.forms import EditCustomerForm, AddCustomerForm, AddAccountForm, TransferForm
...
# app.config["SECURITY_FLASH_MESSAGES"] = True  # Enables flash messages
# app.config["SECURITY_MSG_INVALID_PASSWORD"] = ("Incorrect password. Please try again.", "danger")
# app.config["SECURITY_MSG_USER_DOES_NOT_EXIST"] = ("User not found. Check your email.", "danger")
# app.config["SECURITY_MSG_DISABLED_ACCOUNT"] = ("Your account is disabled.", "warning")
...
if __name__ == "__main__":
    with app.app_context():
        # Kör migrationsuppdatering vid start
        upgrade()

        # Seedar databasen vid behov
        seedData(db)

        # Kör kontroll för misstänkta transaktioner
        # suspicious = detect_suspicious_transactions()
        # if suspicious:
            # print("🚨 Misstänkta transaktioner hittade:")
            # for entry in suspicious:
